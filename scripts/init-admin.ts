...
// Run: bun run scripts/init-admin.ts

import bcrypt from "bcryptjs"
import { db } from "../src/lib/db"

async function main() {
  console.log("🪔 Initializing House of Neelam...")
  ...
  // Seed categories + products + hero + content
  ...
  for (const c of contentSections) {
    const existing = await db.siteContent.findUnique({ where: { section: c.section } })
    if (!existing) {
  ...
  console.log("\n🎉 House of Neelam is ready!")
  console.log("\n📋 Admin Login:")
  console.log(`   Email: ${adminEmail}`)
  console.log(`   Password: ${adminPassword}`)
  console.log("\n   (You can change these from the admin Settings tab)")
}
