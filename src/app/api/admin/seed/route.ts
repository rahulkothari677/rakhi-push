...
// Seed demo data when called by admin
...
async function seedDemoData(force: boolean = false) {
  ...
  // Products - 15 demo Rakhis + roli chawal items
  const products = [
    {
    ...
    for (const p of products) {
      ...
      await db.product.create({
        data: {
          ...
          sku: `RKH-${Math.floor(1000 + Math.random() * 9000)}${Date.now().toString().slice(-3)}`,
        },
      })
    }
  }
  ...
  if (existingSlides === 0 || force) {
    const slides = [
      {
        title: "The Eternal Bond",
        subtitle: "RAKHI COLLECTION 2026",
        description: "Celebrate the sacred thread of love with our handcrafted premium Rakhis. Each piece is a timeless symbol of the unbreakable bond between brother and sister.",
      ...
      },
      {
        ...
        description: "From traditional moli to diamond-studded luxury — discover Rakhis that honor centuries of craftsmanship and devotion.",
      ...
      {
        title: "For Bhaiya & Bhabhi",
        subtitle: "CELEBRATE TOGETHER",
        description: "Exquisite Lumba sets designed to honor both your brother and sister-in-law. Because some bonds deserve double the celebration.",
        image: "/images/hero-3.svg",
        ctaLabel: "View Lumba Sets",
        ctaLink: "shop",
        order: 2,
      },
    ]
  ...
  const aboutContent = await db.siteContent.findUnique({ where: { section: "about" } })
  if (!aboutContent) {
    await db.siteContent.create({
      data: {
        section: "about",
        data: JSON.stringify({
          title: "About House of Neelam",
          body: "House of Neelam was born from a simple yet profound belief — that the bond between a brother and sister deserves to be celebrated with beauty, devotion, and craftsmanship that honors centuries of tradition.\n\nEach Rakhi in our collection is handcrafted by skilled artisans who have inherited their craft through generations. From the sacred threads of moli to the intricate Kundan work on our premium pieces, every detail is a testament to love and dedication.\n\nWe curate our collection with the utmost care, ensuring that every Rakhi you choose is not just an accessory, but a sacred symbol of the eternal bond you share with your sibling.",
    ...
    await db.siteContent.create({
      data: {
        ...
        data: JSON.stringify({
          ...
          body: "Raksha Bandhan, the festival of brothers and sisters, is one of India's most beloved celebrations. The word 'Raksha' means protection, and 'Bandhan' means bond — together, they signify the sacred vow of protection that a brother makes to his sister.\n\nThe tradition dates back thousands of years, with mentions in ancient Hindu scriptures and historical accounts. According to legend, when Queen Karnawati of Chittor sent a Rakhi to Emperor Humayun, he was so moved that he came to her aid, honoring the sacred thread.\n\nToday, sisters tie the Rakhi on their brother's wrist, performing aarti and applying a tilak of roli and chawal. In return, the brother promises to protect his sister and gives her gifts. The ritual is a beautiful expression of love, duty, and the unbreakable bond between siblings.\n\nAt House of Neelam, we honor this sacred tradition by crafting Rakhis that are worthy of the love they represent.",
        }),
      },
    })
  }

  const shippingContent = await db.siteContent.findUnique({ where: { section: "shipping" } })
  if (!shippingContent) {
    await db.siteContent.create({
      data: {
        section: "shipping",
  ...
  if (!contactContent) {
    await db.siteContent.create({
      data: {
        ...
        data: JSON.stringify({
          title: "Get in Touch",
          body: "We'd love to hear from you. Whether you have a question about our Rakhis, need help with an order, or want to discuss a custom design — reach out anytime via WhatsApp or email.",
          phone: "+919504970435",
  ...
  for (const s of settings) {
    ...
    if (!existing || force) {
      await db.siteSetting.upsert({
        ...
        update: force ? { value: JSON.stringify(s.value) } : undefined,
      })
    }
  }
